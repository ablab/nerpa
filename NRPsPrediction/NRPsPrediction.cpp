...
namespace nrpsprediction {
    void nrpsprediction::NRPsPrediction::read_file(std::string file_name) {
        ...
        while (getline(in, s)) {
            ...
            if (nrpparts.size() != 0 && nrpparts[nrpparts.size() - 1].get_orf_name() == orf_name) {
                nrpparts[nrpparts.size() - 1].add_prediction(orf_name_num.second, predict_aminoacids);
            } else {
                nrpparts.push_back(NRPsPart(file_name, orf_name, orf_name_num.second, predict_aminoacids));
            }
        }
