...
nrp::NRPGenerator::NRPGenerator(std::vector<nrp::NRP *> mols) {
...
nrp::NRP *nrp::NRPGenerator::generate(nrp::NRP::NRPType type, int len) {
...
nrp::NRP *nrp::NRPGenerator::generateNRPCycle(int len) {
...
nrp::NRP *nrp::NRPGenerator::generateNRPLine(int len) {
    std::vector<aminoacid::Aminoacids::Aminoacid> amnacid;
    std::vector<std::string> strformula(len);
    std::vector<int> position(len);
...
nrp::NRP *nrp::NRPGenerator::generateNRPBranchCycle(int len) {
...
aminoacid::Aminoacids::Aminoacid nrp::NRPGenerator::genAA() {
    int rv = rand()%sumAA;
    int curSum = 0;
