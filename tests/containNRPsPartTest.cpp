...
namespace nrp {
    ...
    protected:
        ...
        void check_correct_score(matcher::Matcher::Match match, std::vector< nrpsprediction::ORF_Prediction> nrpParts, NRP::NRPType type) {
            nrpsprediction::BGC_Prediction prediction(nrpParts);
            ...
            for (int i = 0; i < matchs.size(); ++i) {
                ...
                if (matchs[pos].first != -1) {
                    nrpsprediction::AAdomain_Prediction amn_pred = nrpParts[matchs[pos].first].getAminoacidsPrediction()[matchs[pos].second];
        ...
        void check_correct_seq(matcher::Matcher::Match match, std::vector< nrpsprediction::ORF_Prediction> nrpParts) {
        ...
        void check_correct_cycle_seq(matcher::Matcher::Match match, std::vector< nrpsprediction::ORF_Prediction> nrpParts) {
            std::vector<std::vector<int> > post(nrpParts.size());
            for (int i = 0; i < post.size(); ++i) {
                post[i].resize(nrpParts[i].getAminoacidsPrediction().size(), -1);
            }
        ...
        void check_seq_match_with_nrp(matcher::Matcher::Match match, std::vector< nrpsprediction::ORF_Prediction> nrpParts) {
            std::vector<std::pair <int, int> > matchs = match.getMatchs();
            for (int i = 0; i < matchs.size(); ++i) {
                if (matchs[i].first != -1) {
                    nrpsprediction::AAdomain_Prediction apr = nrpParts[matchs[i].first].getAminoacidsPrediction()[matchs[i].second];
        ...
        nrpsprediction::ORF_Prediction getSubPart(int bg, int sz, int delta, double &score) {
            ...
            matcher::Score scoring;
            double curs = 0;
            std::vector<nrpsprediction::ORF_Prediction> parts;
            parts.push_back(nrps_part);
            nrpsprediction::BGC_Prediction prediction(parts);
    ...
    TEST_F(ContainNRPsTest, containTrueRandCycleTest) {
        ...
        for (int tst = 0; tst < 1000; ++tst) {
            ...
            nrpsprediction::ORF_Prediction nrps_part = getSubPart(bg, sz, delta, scr);
    ...
    TEST_F(ContainNRPsTest, BranchCycleFakeEnd) {
        ...
        nrpsprediction::ORF_Prediction nrps_part("", "");
        ...
        for (int i = 1; i < aa_sum1.size(); ++i) {
            ...
            using namespace nrpsprediction;
            nrps_part.add_prediction(i, AAdomain_Prediction(i,
                                                            Nrpspredictor2Builder::parse_predictions(ss.str())));
        ...
        std::vector<nrpsprediction::ORF_Prediction> parts;
        parts.push_back(nrps_part);
        nrpsprediction::BGC_Prediction prediction1(parts);
